...
internal static class WorldHandler
{
    ...
    private static void DestroyObject(ObjectDestructible __instance)
    {
        ...
        {
            if (__instance.MapObjectID is "CRATE00" or "CRATE01" && Globals.Random.NextDouble() <= 0.02)
            {
                __instance.GameWorld.SpawnDebris(__instance, __instance.GetWorldPosition(), 0f, [Globals.Random.NextBool() ? "BeachBall00" : "Monkey00"], 1, false);
            }
        }
    }
